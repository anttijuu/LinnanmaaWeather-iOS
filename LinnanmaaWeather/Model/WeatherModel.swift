...
class WeatherModel: ObservableObject {
	...
	func getWeather() async {
		do {
			...
			if let data = try await doGet(from: url) {
				let weather = try parse(from: data)
	...
	private func doGet(from url: URL) async throws -> Data? {
	...
	private func parse(from data: Data) throws -> Weather {
		do {
